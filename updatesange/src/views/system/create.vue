...
<script>
import { formatJson } from '../../api/conversion.js';
import { showMsg } from '../../api/message.js';
import { getSystemDict } from '../../api/system/common.js';
...
export default {
	...
	methods: {
		...
		getList() {
			...
			getSystemDict(this.query_params).then(response => {
				// console.log(response);
				// console.log("getSystemDict = ", response.data)
				var tmpDataInfo = response.data.data;
		...
		handleExport() {
			...
			this.$confirm('是否确认导出所有类型数据项?', '警告', {
				...
				import('../../api/export/Export2Excel').then(excel => {
